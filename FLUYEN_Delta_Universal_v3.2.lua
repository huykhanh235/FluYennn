--[[
    ╔══════════════════════════════════════════════════════════════════╗
    ║           FLUYEN HUD MENU - DELTA UNIVERSAL EDITION             ║
    ║  UI Style: Modern Dark + Orange Accent | All Games | No Errors  ║
    ╚══════════════════════════════════════════════════════════════════╝
]]
...
-- ═══════════════════════════════════════════════════════════════
pcall(function()
    -- Hide from simple detection methods
...
-- ═══════════════════════════════════════════════════════════════
--  THEME - Modern Dark + Orange (like the image)
-- ═══════════════════════════════════════════════════════════════
local Themes = {
    Dark = {
        ...
        accent = Color3.fromRGB(255, 107, 53), -- Orange
...
-- ═══════════════════════════════════════════════════════════════
--  MAIN GUI - MODERN STYLE (like image)
-- ═══════════════════════════════════════════════════════════════
...
-- NO OUTER STROKE (like image)

-- Title Bar (compact, no big header)
...
-- Close X button (top right, like image)
...
-- Tab icons (simple text icons)
local tabIcons = {
    Home = "🏠", Visual = "👁", Combat = "🎯",
    Movement = "🏃", Utility = "🛡", Server = "🌐",
    Fun = "😂", Settings = "⚙"
}
...
-- ═══════════════════════════════════════════════════════════════
--  UI COMPONENTS - CHECKBOX STYLE (like image)
-- ═══════════════════════════════════════════════════════════════
...
-- CHECKBOX (like image - orange checkmark)
...
changelogText.Text = "• v3.2 - Modern UI + Delta Universal\n• v3.1 - Fixed all errors\n• v3.0 - Full HUD Menu\n• v2.0 - Added ESP System\n• v1.0 - Initial Release"
...
-- ═══════════════════════════════════════════════════════════════
--  VISUAL TAB - UNIVERSAL ESP (BillboardGui, no Drawing API)
-- ═══════════════════════════════════════════════════════════════
                ...
                if hrp and hum and hum.Health > 0 then
                    ...
                    if distance <= Values.ESPMaxDistance then
                        ...
                        if espGui then
                            ...
                            local frame = espGui:FindFirstChildOfClass("Frame")
                            if frame then
...
addSlider(pages["Visual"], "Max Distance", "ESPMaxDistance", 100, 5000, 1000)
...
addSection(pages["Combat"], "AIM SETTINGS")
addCheckbox(pages["Combat"], "Enable Aimbot", "Aim", false, function(state)
    if state then
        startAim()
...
addCheckbox(pages["Combat"], "Show FOV Circle", "AimFOV", false, function(state)
    if state then
...
addSection(pages["Combat"], "AIM CONFIGURATION")
addSlider(pages["Combat"], "Aim FOV", "AimFOVValue", 50, 500, 180, function(val)
    if fovCircle then fovCircle.Radius = val end
end)

addSlider(pages["Combat"], "Aim Smoothness", "AimSmoothness", 0, 1, 0)

addCheckbox(pages["Combat"], T("AimPrediction"), "AimPrediction", false)

addSection(pages["Combat"], "OTHER")
...
addSlider(pages["Movement"], "Jump Power", "JumpPowerValue", 50, 300, 100, function(val)
...
addButton(pages["Server"], "Spectate Selected", function()
...
addDropdown(pages["Settings"], "Select Language", {"English", "Tiếng Việt"}, currentLang, function(selected)
